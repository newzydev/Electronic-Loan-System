...
if (isset($_REQUEST['btn_insert'])) {
    ...
    $year_birth_conf = $current_year - $year_of_birth;

    if ($year_birth_conf < 18) {
        $errorMsg = "ไม่สามารถสมัครสมาชิกได้ ตามข้อกำหนดและเงื่อนไขการใช้บริการ ลูกหนี้จะต้องอายุ 18 ปีบริบูรณ์ ขึ้นไปถึงจะใช้งานระบบได้";
    }
    ...
    // บันทึกข้อมูล
    if (!isset($errorMsg)) {
        $sql = "INSERT INTO tbl_member_db(mb_url, mb_id_card_number, mb_phone, mb_firstname, mb_lastname, mb_level, mb_account_number, bk_code, mb_citizen_id_file, mb_book_bank_file, mb_timeinsert)
                VALUE('$mb_url', '$id_card_number_enc', '$mb_phone_enc', '$mb_firstname', '$mb_lastname', '$mb_level', '$mb_account_number', '$bk_code', '$newname1', '$newname2', '$mb_timeinsert')";
...
<body class="bg-light">
    ...
    <div class="background-img">
        ...
        <div class="wrapper">
            ...
            <!-- Content -->
            <?php include('include/register-debtor.php');?>
            <!-- Content -->
